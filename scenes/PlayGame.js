...
class PlayGame extends Phaser.Scene {
    ...
    create() {
        ...
        this.matter.world.on("collisionstart", function (e, b1, b2) {
            ...
            // checking b1 and b2 labels to be "coin"
            if (b1.label == "coins2" || b2.label == "coins2") {
                this.coinCollectScore();
                // calling the method to move the coin elsewhere
                this.placeCoin2();
            }
    ...
    placeCoin2() {
        this.coins2.x = Phaser.Math.Between(game.config.width * 0.2, game.config.width * 0.8);
        this.coins2.y = Phaser.Math.Between(game.config.height * 0.2, game.config.height * 0.8);
    }
    ...
    handleWallCollision(side, bodyA, bodyB) {
        ...
        if (bodyA.color != bodyB.color) {
            ...
            this.FinalScore = this.add.text(game.config.width / 3, game.config.height / 2 + 100, 'Your Score: ' + score, { fontSize: '40px', fill: '#FFF' });
            this.bestScore = this.add.text(game.config.width / 3, game.config.height / 2 + 150, 'Best Score: ' + localStorage.getItem('Best Score'), { fontSize: '40px', fill: '#FFF' });

            if (localStorage.getItem('Best Score') === null) {
                this.bestScore.setText('Best Score: ' + 0);
            } else {
                this.bestScore.setText('Best Score: ' + localStorage.getItem('Best Score'));
            }

            if (score > localStorage.getItem('Best Score')) {
                localStorage.setItem('Third Best Score', localStorage.getItem('Secon Best Score'));
